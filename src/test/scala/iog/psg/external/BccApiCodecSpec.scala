...
import iog.psg.bcc.BccApiCodec._
import iog.psg.bcc.util.{ CustomPatienceConfiguration, DummyModel, ResourceFiles }
 ...
 * bcc package in order to test that these encoders and decoders are
 * available to clients and are not privately scoped to the implementation package
 * (using private[bcc])
 */
class BccApiCodecSpec
    extends AnyFlatSpec
    with Matchers
    with DummyModel
    with ResourceFiles
  ...
  implicit val as: ActorSystem = ActorSystem("bcc-api-test-system")
  ...
  it should "drop nulls in created transaction response" in {
    val ctr = CreateTransactionResponse(
      id = "123",
      amount = QuantityUnit(42000000, Units.entropic),
      insertedAt = None,
      pendingSince = None,
      depth = None,
    ...
    ctr.asJson.noSpaces shouldBe """{"id":"123","amount":{"quantity":42000000,"unit":"entropic"},"direction":"incoming","inputs":[],"outputs":[],"withdrawals":[],"status":"in_ledger"}"""
  }
  ...
  it should "drop nulls Block" in {
    val b = Block(
      slotNumber = 1,
      epochNumber = 2,
      height = QuantityUnit(42000000, Units.entropic),
      absoluteSlotNumber = None
    )

    b.asJson.noSpaces shouldBe """{"slot_number":1,"epoch_number":2,"height":{"quantity":42000000,"unit":"entropic"}}"""
  }

  "Quantity" should "be decoded to Double" in {
    val decodedQU = decode[QuantityUnit[Double]]("""{"quantity":123.45,"unit":"entropic"}""")
    decodedQU.getOrElse(fail("Not decoded")).quantity shouldBe 123.45
  }

  it should "be decoded to Long" in {
    val decodedQU = decode[QuantityUnit[Long]]("""{"quantity":123,"unit":"entropic"}""")
    decodedQU.getOrElse(fail("Not decoded")).quantity shouldBe 123
  }

  it should "be encoded to Double" in {
    val qu = QuantityUnit(123.45, Units.entropic)
    qu.asJson.noSpaces shouldBe """{"quantity":123.45,"unit":"entropic"}"""
  }

  it should "be encoded to Long" in {
    val qu = QuantityUnit(123, Units.entropic)
    qu.asJson.noSpaces shouldBe """{"quantity":123,"unit":"entropic"}"""
  }
