...
import iog.psg.bcc.BccApiCodec.{AddressFilter, CreateTransactionResponse}
...
class BccApiSpec
  ...
  it should "run request with proper params" in {
    ...
    val transactions = api
      .listTransactions(
        ...
        order = BccApi.Order.ascendingOrder,
  ...
  "POST /wallets/{walletId}/migrations" should "submit one or more transactions which transfers all funds from a Sophie wallet to a set of addresses" in {
    api.migrateSophieWallet(wallet.id, walletPassphrase, unUsedAddresses.map(_.id)).executeOrFail() shouldBe jsonFileMigrationsResponse
  }

  it should "return not found" in {
    api.migrateSophieWallet("invalid_address_id", walletPassphrase, unUsedAddresses.map(_.id)).executeExpectingErrorOrFail() shouldBe walletNotFoundError
  }

  ignore should "calculate the exact cost of sending all funds from particular Sophie wallet to a set of addresses" in {
    //this endopoint is "temporarily suspended" in wallet v2021.4.28
    //"GET /wallets/{walletId}/migrations"
    api.getSophieWalletMigrationInfo(wallet.id).executeOrFail() shouldBe jsonFileMigrationCostsResponse
  }

  ignore should "return not found (migrations)" in {
    //this endopoint is "temporarily suspended" in wallet v2021.4.28
    //"GET /wallets/{walletId}/migrations"
    api.getSophieWalletMigrationInfo("invalid_address_id").executeExpectingErrorOrFail() shouldBe walletNotFoundError
  }
  ...
  override implicit val as: ActorSystem = ActorSystem("bcc-api-test-system")
