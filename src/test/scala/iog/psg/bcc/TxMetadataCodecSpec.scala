...
import iog.psg.bcc.util.DummyModel
...
class TxMetadataCodecSpec extends AnyFlatSpec with Matchers with DummyModel {
  ...
  "TxMetadataMapIn encode" should "encode string value to proper json" in  {
    val map: Map[Long, MetadataValue] = Map(0L -> MetadataValueStr("bcc"))
    val metaDataIn: TxMetadataIn = TxMetadataMapIn(map)
    val metaInJsonStr = metaDataIn.asJson.noSpaces

    metaInJsonStr shouldBe """{"0":{"string":"bcc"}}"""
  }
  ...
  it should "encode properly all types to json" in {
    ...
    metaInJsonStr shouldBe """{"0":{"string":"bcc"},"1":{"int":14},"2":{"bytes":"2512a00e9653fe49a44a5886202e24d77eeb998f"},"3":{"list":[{"bytes":"2512a00e9653fe49a44a5886202e24d77eeb998f"},{"int":42},{"string":"1337"}]},"4":{"map":[{"k":{"string":"key"},"v":{"bytes":"2512a00e9653fe49a44a5886202e24d77eeb998f"}},{"k":{"int":14},"v":{"int":42}}]}}""".stripMargin
  }
  ...
  it should "fail on missing type field" in {
    val jsonWithInvalidTypeField = parser.parse("""{"0":"bcc"}""").getOrElse(fail("Invalid json structure"))
  ...
  it should "fail on unsupported type field" in {
    val jsonWithInvalidTypeField = parser.parse("""{"0":{"superdouble":"bcc"}}""").getOrElse(fail("Invalid json structure"))
    val tvMeta = TxMetadataOut(jsonWithInvalidTypeField)
