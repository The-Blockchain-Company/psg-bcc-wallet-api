...
import iog.psg.bcc.BccApi.ErrorMessage
import iog.psg.bcc.jpi.{ AddressFilter, JpiResponseCheck, ListTransactionsParamBuilder }
...
class BccJpiSpec
  ...
  private val addressNotFoundError =
    "iog.psg.bcc.jpi.BccApiException: Message: Addresses not found, Code: 404"
  private val walletNotFoundError =
    "iog.psg.bcc.jpi.BccApiException: Message: Wallet not found, Code: 404"
  ...
  it should "create wallet using account public key" in {
    api
      .createRestoreWithKey(randomWalletName, accountPublicKey, addressPoolGap)
      .toCompletableFuture
      .get() shouldBe wallet.copy(name = randomWalletName, addressPoolGap = addressPoolGap)
  }

  it should "create a wallet, without using mnemonicSecondFactor param" in new CustomInMemoryFixture {
    override val postWalletFieldsToCheck: List[String] =
      List("name", "passphrase", "mnemonic_sentence", "address_pool_gap")
  ...
  "GET /wallets/{walletId}/addresses?state=unused" should "return wallet's unused addresses" in {
    val ids =
      api.listAddresses(wallet.id, AddressFilter.UNUSED).toCompletableFuture.get().asScala.toList.map(_.id)
    ids shouldBe unUsedAddresses.map(_.id)
  }

  it should "return wallet's used addresses" in {
    val ids =
      api.listAddresses(wallet.id, AddressFilter.USED).toCompletableFuture.get().asScala.toList.map(_.id)
  ...
  it should "run request with proper params" in {
    val builder = ListTransactionsParamBuilder
      ...
      .withOrder(iog.psg.bcc.jpi.Order.ASCENDING)
  ...
  it should "return not found error" in {
    ...
    ) shouldBe "iog.psg.bcc.jpi.BccApiException: Message: Transaction not found, Code: 404"
  }
  ...
  it should "return not found" in {
    tryGetErrorMessage(
      api.deleteTransaction("invalid_wallet_id", firstTransactionId)
    ) shouldBe walletNotFoundError
  }

  "POST /wallets/{fromWalletId}/payment-fees" should "estimate fee" in {
    api
      .estimateFee(wallet.id, payments.payments.asJava, withdrawal, txMetadata)
      .toCompletableFuture
      .get() shouldBe estimateFeeResponse
  }
  ...
  it should "estimate fee without metadata and with default withdrawal" in new CustomInMemoryFixture {
    ...
    customApi
      .estimateFee(wallet.id, payments.payments.asJava)
      .toCompletableFuture
      .get() shouldBe estimateFeeResponse
  }
  ...
  "POST /wallets/{walletId}/coin-selections/random" should "fund payments" in {
    api
      .fundPayments(wallet.id, payments.payments.asJava)
      .toCompletableFuture
      .get() shouldBe fundPaymentsResponse
  }
  ...
  "POST /proxy/transactions" should "submit a transaction that was created and signed outside of bcc-wallet" in {
    api
      .postExternalTransaction(txRawContent)
      .toCompletableFuture
      .get() shouldBe jsonFileProxyTransactionResponse
  }

  it should "fail on invalid request body" in {
    tryGetErrorMessage(
      api.postExternalTransaction("1234567890")
    ) shouldBe "iog.psg.bcc.jpi.BccApiException: Message: Invalid binary string, Code: 400"
  }

  "POST /wallets/{walletId}/migrations" should "submit one or more transactions which transfers all funds from a Sophie wallet to a set of addresses" in {
    api
      .migrateSophieWallet(wallet.id, walletPassphrase, unUsedAddresses.map(_.id).asJava)
      .toCompletableFuture
      .get()
      .asScala shouldBe jsonFileMigrationsResponse
  }

  it should "return not found" in {
    tryGetErrorMessage(
      api.migrateSophieWallet("invalid_address_id", walletPassphrase, unUsedAddresses.map(_.id).asJava)
    ) shouldBe walletNotFoundError
  }

  "GET /wallets/{walletId}/migrations" should "calculate the exact cost of sending all funds from particular Sophie wallet to a set of addresses" in {
    api
      .getSophieWalletMigrationInfo(wallet.id)
      .toCompletableFuture
      .get() shouldBe jsonFileMigrationCostsResponse
  }

  it should "return not found" in {
    tryGetErrorMessage(api.getSophieWalletMigrationInfo("invalid_address_id")) shouldBe walletNotFoundError
  }
  ...
  it should "return error" in {
    tryGetErrorMessage(api.listStakePools(-1)) shouldBe "iog.psg.bcc.jpi.BccApiException: Message: Invalid stake parameter, Code: 400"
  }
  ...
  it should "return not found" in {
   tryGetErrorMessage(api.joinStakePool("invalid_wallet_id", stakePoolId, walletPassphrase)) shouldBe "iog.psg.bcc.jpi.BccApiException: Message: Not found, Code: 404"
  }
  ...
  it should "return not found" in {
    tryGetErrorMessage(api.quitStakePool("invalid_wallet_id", walletPassphrase)) shouldBe "iog.psg.bcc.jpi.BccApiException: Message: Not found, Code: 404"
  }
  ...
  override implicit val as: ActorSystem = ActorSystem("bcc-api-jpi-test-system")

  private def getCurrentSpecAS: ActorSystem = as

  sealed trait CustomInMemoryFixture
      extends Configure
      with ModelCompare
      with ScalaFutures
      with InMemoryBccApi
    ...
    lazy val customApi: jpi.BccApi = JpiResponseCheck.buildWithPredefinedApiExecutor(inMemoryExecutor, as)
  }
