...
import iog.psg.bcc.BccApi.{ BccApiRequest, BccApiResponse, ErrorMessage, Order }
import iog.psg.bcc.BccApiCodec.{ GenericMnemonicSecondaryFactor, GenericMnemonicSentence }
import iog.psg.bcc.jpi.BccApiException
...
trait InMemoryBccApi {
  ...
  implicit final class InMemoryExecutor[T](req: BccApiRequest[T]) {
    def executeOrFail(): T =
  ...
  implicit final class InMemoryFExecutor[T](req: Future[BccApiRequest[T]]) {
    def executeOrFail(): T =
  ...
  val inMemoryExecutor: ApiRequestExecutor = new ApiRequestExecutor {
    override def execute[T](
      request: BccApi.BccApiRequest[T]
    )(implicit ec: ExecutionContext, as: ActorSystem): Future[BccApiResponse[T]] = {

      implicit class JsonRespectable[A](resp: A) {
        def toJsonResponse()(implicit enc: io.circe.Encoder[A]): Future[BccApiResponse[T]] =
          request.mapper(
            HttpResponse(entity =
      ...
      def noContentResponse(): Future[BccApiResponse[T]] =
        request.mapper(HttpResponse(status = StatusCodes.NoContent))

      def notFound(msg: String): Future[BccApiResponse[T]] = {
      ...
      def badRequest(msg: String): Future[BccApiResponse[T]] = {
      ...
      def checkValueOrFail[A](jsonValue: A, expectedValue: A, fieldName: String): Future[Unit] =
        ...
        else Future.failed(new BccApiException(s"Invalid $fieldName", "400"))
      ...
      def checkAddressesField(json: Json): Future[Unit] = {
        ...
        else Future.failed(new BccApiException(s"Invalid address", "400"))
      }
      ...
      def checkMetadataField(json: Json, fieldsToCheck: List[String]): Future[Unit] = if (
        ...
        for {
          metadata <- {
            ...
            if (metaResults.isEmpty) Future.failed(new BccApiException(s"Invalid metadata", "400"))
