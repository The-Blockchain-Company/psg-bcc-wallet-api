...
import iog.psg.bcc.TxMetadataOut
...
trait DummyModel { self: Assertions =>
  ...
  final val inAddress = InAddress(
    ...
    amount = Some(QuantityUnit(quantity = 42000000, unit = Units.entropic)),
    id = "1423856bc91c49e928f6f30f4e8d665d53eb4ab6028bd0ac971809d514c92db1",
    index = 0
  )

  final lazy val outAddress =
    OutAddress(address = addressIdStr, amount = QuantityUnit(quantity = 42000000, unit = Units.entropic))
  ...
  final lazy val wallet = Wallet(
    ...
    addressPoolGap = 20,
    balance = Balance(
      available = QuantityUnit(42000000, Units.entropic),
      reward = QuantityUnit(42000000, Units.entropic),
      total = QuantityUnit(42000000, Units.entropic)
    ),
    delegation = Some(
      Delegation(
  ...
  final lazy val networkParameters = NetworkParameters(
    ...
    minimumUtxoValue = QuantityUnit(42000000, Units.entropic),
  ...
  final lazy val payments = Payments(Seq(Payment(unUsedAddresses.head.id, QuantityUnit(100000, Units.entropic))))

  final lazy val estimateFeeResponse = {
    val estimatedMin = QuantityUnit(quantity = 42000000, unit = Units.entropic)
    EstimateFeeResponse(
  ...
  final lazy val uTxOStatistics = UTxOStatistics(
    total = QuantityUnit(quantity = 42000000, unit = Units.entropic),
    scale = "log10",
    distribution = Map(
      "10" -> 1,
      "100" -> 0,
