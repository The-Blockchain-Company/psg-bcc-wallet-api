...
public class ListTransactionsParamBuilder {
    ...
    public Enumeration.Value getOrder() {
        ...
        Enumeration.Value result = BccApi.Order$.MODULE$.Value(Order.DESCENDING.name().toLowerCase());
        if(order != null) {
            result = BccApi.Order$.MODULE$.Value(order.name().toLowerCase());
        }
        return result;
    }
