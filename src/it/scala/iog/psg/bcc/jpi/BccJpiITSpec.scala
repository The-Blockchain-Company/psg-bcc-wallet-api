...
class BccJpiITSpec extends AnyFlatSpec with Matchers with Configure with ModelCompare with BeforeAndAfterAll {
  ...
  private lazy val sut = new JpiResponseCheck(new BccApiFixture(baseUrl).getJpi, timeoutValue, timeoutUnits)
  ...
  "Jpi BccAPI" should "allow override of execute" in new TestWalletFixture(1) {
    ...
    val expectedWallet = Wallet(
      ...
      addressPoolGap = 10,
      balance = Balance(
        available = QuantityUnit(1, Units.entropic),
        reward = QuantityUnit(1, Units.entropic),
        total = QuantityUnit(1, Units.entropic)
      ),
      delegation = Some(
        Delegation(
