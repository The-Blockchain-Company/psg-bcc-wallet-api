...
import iog.psg.bcc.BccApi.BccApiOps._
import iog.psg.bcc.BccApi._
import BccApiCodec.{AddressFilter, CreateTransactionResponse, GenericMnemonicSentence, MetadataValueArray, MetadataValueLong, MetadataValueStr, Payment, Payments, QuantityUnit, SyncState, TxMetadataMapIn, TxState, Units}
...
object BccApiTestScript {
  ...
  def main(args: Array[String]): Unit = {
    ...
    Try {
      val baseUri = args.headOption.getOrElse(throw new IllegalArgumentException("Pass the base URL to the bcc wallet API as a parameter"))
      ...
      val entropicToTransfer = 10000000
      ...
      val api = new BccApiImpl(baseUri)
      ...
      if (netInfo.syncProgress.status == SyncState.ready) {
        ...
        if (fromWallet.balance.available.quantity > 2) {
          ...
          val payments = Payments(
            Seq(
              Payment(paymentTo.id, QuantityUnit(entropicToTransfer, Units.entropic))
            )
          )
          ...
          println("Use https://testnets.bcc.org/en/bcc/tools/faucet/ to lodge TESTNET credit to below address")
  ...
  private def unwrap[T: ClassTag](apiResult: BccApiResponse[T]): T = unwrapOpt(Try(apiResult)).get

  private def unwrapOpt[T: ClassTag](apiResult: Try[BccApiResponse[T]]): Option[T] = apiResult match {
